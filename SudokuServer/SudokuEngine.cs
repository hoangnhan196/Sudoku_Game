...
namespace SudokuServer.Game
{
    public class SudokuEngine
    {
        ...
        public bool CheckMove(int row, int col, int value)
        {
            ...
            // Ô này đã có số (số gốc của puzzle) thì không cho đè
            if (PlayerBoard[row, col] != 0) return false;

            // ✅ Kiểm tra theo luật Sudoku: không trùng hàng, cột, box 3x3
            // Tạm thời xóa ô đó để IsValid không tự so sánh với chính nó
            int backup = PlayerBoard[row, col];
            PlayerBoard[row, col] = 0;
            bool valid = IsValid(PlayerBoard, row, col, value);
            PlayerBoard[row, col] = backup;
        ...
        public bool IsGameFinished()
        {
            for (int r = 0; r < 9; r++)
            {
                for (int c = 0; c < 9; c++)
                {
                    ...
                    // Còn ô trống → chưa xong
                    if (val == 0) return false;

                    // ✅ Kiểm tra số tại ô này có hợp lệ không (không trùng hàng/cột/box)
                    PlayerBoard[r, c] = 0;
                    bool valid = IsValid(PlayerBoard, r, c, val);
                    PlayerBoard[r, c] = val;
